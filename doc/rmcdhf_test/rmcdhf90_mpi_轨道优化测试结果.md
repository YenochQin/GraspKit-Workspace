...
数值结果来自 `rmcdhf_test/test/rmcdhf_orbopt/RESULTS.md` 中已归档的 AS2 摘要。仓库当前没有保存这些运行的完整 `trace/` 输出目录，因此本文档仅引用已记录的汇总数据，不将未保存的运行细节补写为已验证事实。
...
已经 Cl I AS1–AS5 的链式实验，但尚未完成 `-0.2/-0.5/-0.8` 全矩阵和
多次重复，因此不应固化为生产默认值。
...
6. Cl I、strict SCF、B5/B6 和已选 MPI 对照已补齐，但在完成 B7、
   阻尼全矩阵、重复性与节点稳定性验收之前，本实施仍是诊断/实验版，
   不是已验收的最终生产修复。
