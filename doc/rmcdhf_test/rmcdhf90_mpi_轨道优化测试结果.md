...
以及 2026-08-25 新完成的 B4 阻尼矩阵汇总 CSV。仓库只保存小型汇总，
完整 trace 仍位于外部实验目录，因此本文档不将未归档的运行细节补写为已验证事实。
...
runner 使用归档 AS2 CSF 输入和前一 AS 波函数，调用当前 `rangular_mpi`、`rwfnestimate` 和 `rmcdhf_mpi`。为避免默认超额订阅，runner 将 `OMP_NUM_THREADS` 和 `OPENBLAS_NUM_THREADS` 默认设为 1，允许通过 `GRASP_OMP_THREADS` 显式覆盖。
...
- 接受后最大平均半径因子降到约 `1.56–1.82`；
- 接受后未观测到节点变化；
- 预期 J 顺序保持不变。

这是目前支持“负阻尼能抑制实际接受更新过冲”的最强实验证据。`-0.5`
已经 Cl I AS1–AS5 的链式实验以及 `-0.2/-0.5/-0.8` 三体系矩阵，
但尚未完成每个变体的多次重复，因此不应固化为生产默认值。

### 4.1 2026-08-25 阻尼全矩阵

现已使用同一构建、4 MPI ranks、每 rank 12 threads 完成 Cl I AS1、
...
`-0.8` 适合强稳定性对照，但不支持设为默认。阻尼全矩阵已完成，但多次重复
和 Ni I 节点稳定性仍未完成。

### 4.2 与本地 NIST ASD CSV 的对比
...
性能记录表明，在 48 核参考主机上，已测组合中最快的是 4 MPI ranks、每 rank 12 OpenMP 线程。纯 24-rank MPI 通信占比较高；将所有 rank 强制为单线程则未充分利用带线程的 BLAS。
...
| 已满足最终生产验收 | Cl I 谱序与部分 MPI 一致性已通过，但节点稳定性和完整重复矩阵未完成 | 不成立 |
...
6. Cl I、strict SCF、B5/B6、阻尼全矩阵和已选 MPI 对照已补齐，但在完成 B7、
   重复性与节点稳定性验收之前，本实施仍是诊断/实验版，
   不是已验收的最终生产修复。
