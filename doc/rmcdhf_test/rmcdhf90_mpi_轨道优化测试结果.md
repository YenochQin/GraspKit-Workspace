...
这些结果证明效应具有方向性：原始 `nl` 单侧选择会产生不平衡松弛并改变精细结构顺序，`nl-` 单侧在这两组算例中没有造成错序。成对选择是已验证的稳健约束，但数据不支持“任意缺少伙伴都必然错序”的更强说法。
...
596 的逐字节重复性，当时把下一步定为审计 AS2 新增轨道的 MPI 状态传递。后续
Jobs 635/636 已补出更早的 AS1 基线贡献：Ni I 在优化 AS1 + TF AS2 时已经倒序，
因此本段只保留为当时决策记录，当前结论以第 35 节为准。

## 结论分层与后续主线

本节记录 Job 636 之前的阶段判断。当时已知不优化链正确、逐个优化 AS2 新增轨道
仍会改变错误间隔，因此审计重点放在 `IMPROVmpi`。Job 636 随后证明 Ni I 在优化
AS1 + TF AS2 时已经倒序，撤销了“根因只位于 AS2 新增轨道”的范围判断；当前
因果结论和生产策略见第 35 节。
...
当时的后续主线是逐步审计 `SETLAGmpi → SOLVE → IMPROVmpi → DAMPCK/DAMPOR →
ORTHY → MATRIXmpi/NEWCOmpi`。该审计发现并验证了 QDIF、Gatherv、稀疏索引及
收敛/回退问题，但这些代码因素都没有解释有效输入上的精细结构异常；护栏仍只作
诊断辅助，不能替代 balanced 物理输入和基线验收。
...
> 历史记录更正：本节 Ni 默认索引 `3,4,7` 实际对应 J=1,2,3；真实输入中的
> J=2,3,4 首根应为 `4,7,8`。当时抽象匹配测试未核验实际 J 映射，详见第 31 节。
...
它们都在 $J=2$ block（此前误记为 J=3）；J=2,3,4 首根的正确全局索引是
`4,7,8`，当时改为 `3,4,7` 仍错误。Cl I 的 `1,2` 保持不变。
另一个问题是目标态字符串中的逗号未在 trace 中引用，导致所有算例的
...
当时将默认目标态改为 `3,4,7`，为含逗号的 trace 字段增加 CSV 引号，并把状态表
...
Job 640 结果目录为 `data/rmcdhf_test_data/results/round-guard-minimal-640/`。三组
trace 均为 70 列、CSV 记录完整；控制行确认 Ni I、Ni/Ca-like 目标态为 `3,4,7`，
Cl I 为 `1,2`。

| 算例 | 回退/接受 | 配置集合最低重叠 | 配置集合顺序回退 | 结果 |
|---|---:|---:|---:|---|
| Ni I AS2 | 4 / 0 | 0.7740 | 0 | RMCDHF 退出 1，超过回退上限 |
| Ni/Ca-like AS2 | 3 / 32 | 0.8871（回退轮） | 3 | 超时退出 124，无最终 CSV |
| Cl I AS1 | 0 / 100 | 0.999323 | 0 | RMCDHF 退出 0；严格检查未通过 |

第 31 节复核后撤回“目标范围已正确”和“失败已证实来自物理目标态失稳”的解释：
Ni 的 `3,4,7` 实际覆盖 J=1,2,3，遗漏 J=4；构建仍含 Job 609 的稀疏索引回归，
Ni I 第 0 轮就已有异常能谱。Ni/Ca-like 的三次回退实际在第 4、5、8 轮，
不能作为目标 ³F 的能量顺序验证。Cl I 生成了 CSV，但 100 轮末严格收敛未通过。

这些记录保留为输出格式与控制路径证据，不能据此调整物理态重叠阈值或宣称恢复顺序。

## 30. 目标态重叠按 assignment 列修正（2026-09-13）

复核 Job 640 的护栏实现后发现，目标态索引表示上一轮的物理根（Hungarian
assignment 的旧根列），而原实现却按候选矩阵行号取重叠。发生根交换时，原实现
可能把辅助根的低重叠误判为目标根，并错误回退整轮。现已改为按 assignment 的旧根
列取目标重叠；目标能量映射也复用同一列到行关系。

同时新增 `round_target` trace 事件，逐目标记录旧全局索引、候选行、块内位置、
旧/新能量和实际 CI 重叠，以便区分物理目标根与辅助根。离线 assignment 测试增至
六项并全部通过，`build/bin/rmcdhf_mpi` 重新构建成功。

为避免重复 Job 640 的三组算例，原计划只运行 Ni I AS2 的 Job 641。该作业因新脚本
提前创建结果目录，被 `run_data_case.sh` 的防覆盖检查以 `runner_exit=2` 拒绝，未启动
RMCDHF，不能作为物理结果。已修正目录创建时序并提交 Job 642；完成后只比较其
`round_target`/`round_decision` 与 Job 640 的 Ni I trace，再决定是否需要调整阻尼策略，
在此之前不扩大测试矩阵。

## 31. Job 642：有效运行，初始 CI 与目标映射仍错误（2026-09-13）

结果目录：`data/rmcdhf_test_data/results/round-target-column-ni-642/`。
`runner_exit=0`（预期失败模式）、`rmcdhf_exit=1`、`convergence_exit=not_run`。
RMCDHF 实际运行四轮，全部以 `state_overlap` 拒绝，第四次超过回退上限；无最终
能级 CSV。trace 310 行、70 列完整，12 条逐目标记录已离线整理为 `target_audit.csv`。

| 配置索引 / 实际 J | 四轮 old→current | 四轮重叠 |
|---|---|---|
| 3 / J=1 | 3→3 | 0.992366 / 0.992388 / 0.992366 / 0.992343 |
| 4 / J=2 | 4→5 | 0.780247 / 0.826530 / 0.811409 / 0.805601 |
| 7 / J=3 | 7→7 | 0.896416 / 0.897388 / 0.900412 / 0.900945 |

旧根 4（J=2）触发 0.85 门槛；列匹配修正改善了诊断，但没有解除失败。
两个 Ni 输入均有 J=0,1,2,3,4 五块，各选 2,1,3,1,2 根，所以 J=2,3,4 的首根
应为 `4,7,8`。此前 `3,4,7` 正确的说法错误，脚本默认值现已修正，后续 trace
还会记录实际 `2J` 和 parity。

更关键的是 Job 642 在优化开始前已有严重能谱偏差。与 Job 631 `as2-all_new`
相比，`isodata`、`rcsf.inp`、`previous.w`、`rwfn.inp` 的 SHA-256 全部相同，
但第 0 轮 J=2/3/4 首根分别偏低 66.036237932、56.946072828、59.809361611 Eh。
九个初始根的完整对照见 `initial_energy_audit.csv`，这不是 SCF 更新造成的首次差异。

已追到当前构建仍含的错误稀疏索引：`MATRIXmpi` 只给本进程拥有列填写 `IENDC`，
`EMT/IROW` 是连续的本地列数据；相邻全局列 `ICOL-1` 属于其他进程，不能作为
本地起点。已将 `SPICMVmpi` 和 `INIESTmpi` 恢复为沿本地列累计偏移。

用生产 Fortran 例程运行一项小型 CTest `mpi90_local_sparse`，稠密已知矩阵作为
参考、归约接口受控替换，无 MPI/Slurm 计算。修正前 2/3/4 份分区均失败（乘积
最大绝对误差 12.614/8.529/6.857）；修正后乘积误差不超过 4.44e-16，packed
条目误差为 0，特征对残差约 1.76e-15。1 份及 46 份分区也覆盖，但 8×8 矩阵
的 46 份仅检查空分区，不代表生产 MPI 验收。正式 `rmcdhf_mpi` 已重建成功。

输出复核也纠正此前“危险波函数不落盘”的断言：`rwfn.out` 与被拒绝的 iter004
哈希相同，前三次回退快照彼此相同但与之不同。代码在判定前落盘、达到回退上限时
又先报错退出；`rmix.out` 同样在判定前写出。这些文件不是可复用的已接受结果，
失败出口的持久化时序尚需修正。

本次只修复代码、做针对性的离线检查和更新记录，未提交新作业。下一步优先核实修正
构建的第 0 轮与 Job 631 已有初始 CI 一致，再处理回退输出和跨轮目标身份，随后才
继续轨道优化恢复。即使初始 CI 恢复，Job 631 的优化 AS1 已偏离 NIST 的问题仍在，
不能把消除本次底层回归写成原始物理问题已经解决。

## 32. 回退输出事务修正与 Job 643（2026-09-13）

回退分支现在在内存恢复后直接用已接受快照重写 `.mix`，避免调用 `MATRIXmpi` 造成
额外对角化或再次阻尼；`.rwf` 与 `.mix` 均在回退上限触发前恢复。构建、稀疏矩阵
CTest、六项离线目标态测试和空白检查均通过。Job 643 已提交，用于确认修正后的
46-rank 初始 CI 与 Job 631 第 0 轮一致，并核对失败出口文件；作业结果待完成后再
记录，当前不将其视为物理验收。

## 33. Job 643 结果（2026-09-14）

Job 643 的 RMCDHF 本体退出 0，运行 100 轮；wrapper/严格检查退出 1。第 0 轮
九个 CI 能量与 Job 631 逐根完全一致，确认生产 46-rank 稀疏矩阵路径已恢复。
目标 4/7/8 在 100 轮内始终映射到自身，最低相邻轮重叠为
0.991565/0.993835/0.994123，100 轮全部接受且无顺序回退。

本结果不通过物理或收敛验收：能量从第 25 轮起已达到判据，但轨道判据一直为假，
最终 `$^3F_2<{}^3F_3<{}^3F_4$`，间隔为 4244.23/8440.97 cm$^{-1}$。因此
整轮身份护栏能够阻止未跟踪的根丢失，却不能修正从 AS1/初始目标继承的错误能序。
Job 643 没有回退，不能据它宣称失败出口文件写回已动态验收。

后续代码已让合法 root swap 的目标 row 在接受轮后持续到下一轮，拒绝轮不更新；
离线状态测试由六项增至八项并通过。严格检查器现在把 Job 643 明确报告为“达到
100 轮上限仍未严格收敛”。下一次物理计算应以正确 AS1/TF 波函数为基线并缩小到
目标 `$^3F_J$` 状态集合，不再重复 Job 643 的全状态、错误 AS1 路径。
为下一步只准备一个算例：归档正确 AS1 波函数、五个 J 块各取首根的 reduced-ASF、AS2 新轨道优化和
整轮身份护栏。五个 J block 各选一个根，因此 `$^3F_{2,3,4}$` 的目标索引为
`3,4,5`。脚本已完成 shell 语法及 prepare-only 输入检查，并提交为 Job 644。
它复用 Job 590 的正确 AS1、固定阻尼和节点进度护栏，
并开启能序回退及严格三重收敛；结果待作业结束后记录。

Job 644 未进入 SCF：脚本错误地用单分量 `optimized` 轨道列表配合强制 balanced-pair
检查，三组伙伴轨道均被判为 `unbalanced_selection`，RMCDHF 退出 1。该作业没有物理
结果。脚本已改用成对的 `balanced` 列表，并在末尾返回实际 runner 退出码；修正后
提交为 Job 645。

## 34. Job 645 结果（2026-09-14）

Job 645 为 `COMPLETED 0:0`，耗时 35:16；runner、RMCDHF 和严格检查均退出 0。
62 轮后 `convg_orbital/convg_energy/convg_strict/convg_final` 全为 true，
`strict_streak=2`。目标 3/4/5 在所有轮次都映射到自身，最低 CI 重叠分别为
0.992726/0.992766/0.992726，62 轮全部接受。第 3 轮仅有一次已知的 `5p-`
节点进度拒绝。

最终为 `$^3F_4<{}^3F_3<{}^3F_2$`，累计间隔 1374.81/2290.63 cm$^{-1}$，
相对 NIST 1332.164/2216.550 cm$^{-1}$ 偏高约 42.65/74.08 cm$^{-1}$。
结果与既有 reduced-ASF Job 602 的 1374.78/2290.42 cm$^{-1}$ 一致到约
0.21 cm$^{-1}$。两者前 18 轮的 1671 条共有生产事件中，选取的 19 个身份与数值
字段完全一致；Job 645 的
62 轮 round 又全部接受。因此本作业证明该 balanced/reduced-ASF 路径可达到严格
收敛，并未证明 round guard 改变或修复了物理路径。

严格收敛相对 Job 602 仍改变了小量径向尾部：新增轨道最终重叠均高于 0.999923，
平均半径比最大为 1.01274（`5d-`）。更敏感的离线节点阈值在 `5d-/5d/6s` 上
从 3/3/6 变成 2/2/5，但生产 `COUNT` 在 Job 602 第 18 轮已报告 2/2/5；所以这是
低幅尾部稳定化，不是物理节点修复。

与相同正确 AS1 的 full-ASF Job 601 比较，目标间隔差别只有约 1.1/1.6 cm$^{-1}$；
因此 reduced-ASF 也不是正确能序的必要原因。把 Jobs 635/636 纳入后，Ni I 的严格
因果链为：`_NV AS1 + TF AS2` 正确；错误优化 AS1 + TF AS2 已倒序；在错误 AS1
上优化 AS2 会进一步放大倒序。共享 AS1 的早期 B1/B3 单因素又显示，仅优化正号
`nl` 会错序，而 `nl-/nl` 成对优化恢复正确顺序。现有最强解释是原始 `nl` 单侧
松弛使不同 J 态得到不平衡的相关能降低，随后错误 AS1 被下一活性空间继承。

TF 初猜表现好是有限模型中的公共中心场/误差抵消现象：它避免了针对有限 EOL 状态
和截断 CSF 空间的过度轨道松弛，但不是普遍优于优化。Job 636 中 TF AS2 接在错误
优化 AS1 上仍然倒序，证明必须审查整条 AS 基线。Job 645 的 NIST 累计偏差仍为
+42.65/+74.08 cm$^{-1}$；full/reduced-ASF 状态集合与严格收敛只改变约 0--2 cm$^{-1}$，
不能解释这部分偏差。

当前 CI 护栏的证据边界也需保留：Jobs 643/645 均无 root swap、无 round rollback，
且 Job 645 每块只选一根。它能诊断低重叠或突然能序变化，但不能把已错序 AS1 恢复，
也不能捕捉每轮变化很小、长期累积的组态漂移。现有有效数据中的目标仍是同一
`^3F_J`（主导权重约 0.90--0.99）；目前尚未用有效路径复现“直接变成其他组态”。
这里的 overlap 是相同 CSF 排列下 CI 系数向量的点积；轨道组已经变化且未做双正交
变换，不能称为严格的多电子 ASF 重叠。

跨体系结果限制了结论强度：Ni I、Ni/Ca-like 和 Cl I 同夹具的 B1 `nl` 单侧路径
均出现错序，B2/B3 对照恢复/保持正确顺序；Fe I Job 621 的单侧路径却
保持正确 `^5D_J` 顺序。因此不完整伙伴选择是已证实的高风险触发条件，不是所有
体系必然失败的充分条件。生产上应强制完整性检查并以 balanced 为默认工作流约束，
精度仍要逐体系与固定 TF/RCI 锚点比较。

现有结果已足以停止重复 Ni I 回归。剩余实现缺口是固定初始 TF/AS 子空间锚点、
主角重叠与参考组态权重门槛、以及按活性空间整体回退；相邻轮 CI overlap 无法检测
缓慢累计的组态漂移。`.rwf/.mix` 回退写回也尚未在有效物理作业中动态触发，后续只在
自然出现回退时验收。剩余约 43/74 cm$^{-1}$ 的 Ni I NIST 偏差应转入相关空间和
Breit/QED 等物理模型单因素分析，不继续通过阻尼或 overlap 阈值调参。
