...
2. 原始输入只优化 `nl`、固定对应 `nl-` 时，会使不同 $J$ 块获得不平衡的轨道松弛；后续 B1/B2/B3 对照已确认该效应具有方向性：`nl` 单侧在 Ni I 和 Ni/Ca-like 中触发错序，`nl-` 单侧没有，成对优化恢复预期顺序。因此不能概括成“任意单侧选择必然失败”，但生产输入应使用已验证的成对约束。
...
本文档同时保留原始目标方案和按日期追加的实施记录。截至 2026-09-14，P0 诊断、
B0--B8 主要对照、阻尼矩阵、strict SCF、固定轨道 RCI、AS1/AS2 贡献分解、
46-rank 稀疏矩阵回归以及整轮身份/回退状态机均已有结果。2026-09-15 已补齐固定参考
锚点、组态/径向/间隔验收、AS 级事务回退及只继承已验收快照的生产入口。仍待自然
物理算例覆盖的是动态 round rollback 输出；四体系 balanced 精度验证属于后续科学
验收，不再是生产安全状态机的实现缺口。串行/MPI 逐轮对照不作为本方案验收项。
当前结论与完成度见
第 35 节。具体状态也可对照
...
> 2026-09-13 更正：本节原先把 `IENDC` 当成全局累计列索引的解释错误。
> `MATRIXmpi` 只读入本进程拥有列的局部端点；下文提出的两个改动导致了
> Job 609 及后续护栏构建的回归。已按真实存储方式修正并用生产例程验证，见第 31 节。
...
3. **严格 AS1/AS2 归因保持不变。** 636 的固定 RCI 已显示 Ni I 在“优化 AS1+
   ...
   AS1 先压缩间隔，AS2 全量优化再触发反转。当时据此采用固定已验证 AS1/TF
   基线并实施整轮回退、目标态身份跟踪和三重收敛；Jobs 643/645 已完成这些控制
   路径的首轮验证，最终生产结论见第 35 节。

这一步已经足以决定当时的下一步方向，不需要再重复 Jobs 631/632/635/636。其后已
实现“候选更新→整轮接受/回退”状态机，并用目标 $^3F_J$ 的 CI 向量重叠和相对
能量验收；Jobs 643/645 进一步证实，仅增加根编号检查不能覆盖 Ni I 的 AS1 已
倒序情形。
...
> 历史记录：本节的 Ni 默认集合 `3,4,7` 仍然错误，实际对应 J=1,2,3。
> 第 31 节根据真实 stdout/trace 更正为 `4,7,8`；本节五项抽象测试没有验证实际输入映射。
...
1. 脚本把 Ni I 和 Ni/Ca-like 的目标态误设为 `4,5,6`，这三个索引都属于
   $J=2$ block；此前误写成 J=3。当前输入中 J=2,3,4 的首根全局索引应为
   `4,7,8`，当时改为 `3,4,7` 仍然错误（见第 31 节）。Cl I 的 `1,2` 保持不变。
...
当时的修改为：默认目标集合改为 `3,4,7`，trace 输出对含逗号字段使用 CSV 引号，
状态表把 `convergence_check` 尚未生成正确分类为后处理失败，而不是严格收敛失败。
修正后重新构建和离线测试已通过；下一次测试必须重新生成有效 trace 后再评价目标态
护栏，不能复用 Job 639 的结果。

## 29. Job 640：运行记录与撤回的物理解释（2026-09-13）

Job 640 使用当时配置的目标态集合和已修正的 CSV 输出，结果目录为
`data/rmcdhf_test_data/results/round-guard-minimal-640/`。三组 trace 均为 70 列且
没有 schema 错位，控制行确认 Ni I、Ni/Ca-like 使用 `3,4,7`，Cl I 使用 `1,2`。

| 算例 | 回退/接受 | 配置集合最低重叠 | 配置集合顺序回退 | RMCDHF/后处理结果 |
|---|---:|---:|---:|---|
| Ni I AS2 | 4 / 0 | 0.7740 | 0 | RMCDHF 退出 1，超过 3 次回退上限 |
| Ni/Ca-like AS2 | 3 / 32 | 0.8871（回退轮）；接受轮约 0.999 | 3 | 90 分钟超时退出 124，未形成最终 CSV |
| Cl I AS1 | 0 / 100 | 0.999323 | 0 | RMCDHF 退出 0，100 轮达到上限，严格收敛检查未通过 |

第 31 节复核后撤回此前两项解释：`3,4,7` 实际是 J=1,2,3，不能证明 J=2,3,4
目标范围正确；这批构建仍有 Job 609 的稀疏存储回归，Ni I 在第 0 轮已产生异常
能谱。因此不能把低重叠归因为真实目标态的优化失稳，也不能把 Ni/Ca-like 第 4、5、8
轮的三次 `energy_order` 回退当作目标 ³F 序列的验证。

Cl I 生成了 CSV，但达到 100 轮上限且严格收敛未通过，不能称为物理验收通过。
Job 640 只保留输出格式和控制路径证据；优先修复底层矩阵与目标映射，不调整阻尼门槛，
不重复这批作业。此前“阻止错误波函数落盘”的说法也需要修正，见第 31 节的输出检查。

## 30. 目标态重叠列映射修正（2026-09-13）

Job 640 之后复核发现，`GRASP_ROUND_TARGET_STATES` 的索引是上一轮物理根的全局
索引，应对应 Hungarian assignment 的旧根列；原实现按候选行号筛选目标。根交换时
这两者不同，会将辅助根重叠用于目标态门槛，导致错误回退。实现已改为按 assignment
旧根列选取目标重叠，并让目标能量沿同一列到行映射计算。

trace 新增逐目标 `round_target` 行：`index` 为旧目标全局索引，`index2` 为匹配的
候选行，`position` 为块内候选位置，`energy_old`/`energy_candidate`/`overlap`
为映射后的数值，`detail` 记录块映射。离线 assignment 测试增至六项并通过，正式
`build/bin/rmcdhf_mpi` 已重建。

验证只运行 Ni I AS2，没有重复 Job 640 的 Cl I 和 Ni/Ca-like。Job 641 因包装脚本
提前创建结果目录，被 `run_data_case.sh` 的防覆盖检查以 `runner_exit=2` 拒绝，未启动
RMCDHF，不能作为物理结果。修正目录创建时序后已提交 Job 642；完成后先审计逐目标根
匹配和重叠，再决定是否需要改动负阻尼或扩大测试，不能在缺少该证据时调整阈值。

## 31. Job 642：初始 CI 回归定位与目标映射更正（2026-09-13）

Job 642 已实际运行 46 ranks，`runner_exit=0`、`rmcdhf_exit=1`、
`convergence_exit=not_run`，四轮均因 `state_overlap` 回退，第四次超过上限。
wrapper 的 0 来自预期失败模式，不代表计算通过；没有最终能级 CSV。
trace 共 310 行、70 列，包含完整的 12 条 `round_target` 记录。

stdout 的块序列是 J=0,1,2,3,4，分别选 2,1,3,1,2 个根。因此实际映射为：

| 已配置旧索引 | 实际 J | 四轮匹配到的当前索引 | 四轮 CI 重叠 |
|---|---:|---:|---|
| 3 | 1 | 3 | 0.992366 / 0.992388 / 0.992366 / 0.992343 |
| 4 | 2 | 5 | 0.780247 / 0.826530 / 0.811409 / 0.805601 |
| 7 | 3 | 7 | 0.896416 / 0.897388 / 0.900412 / 0.900945 |

触发门槛的是 J=2 块的旧根 4，映射到当前根 5；J=4 的首根是全局索引 8，
未纳入本作业的门槛。列匹配修正改变了最低重叠值，但未解决失败。
Ni/Ca-like 的相同块序列也已由其 Job 640 stdout 核实；两个脚本的 Ni 默认集合
已改为 `4,7,8`，后续 trace 的 detail 增加 `2J` 和 parity。

更早的异常已出现在轨道更新前：Job 642 与 Job 631 `as2-all_new` 的 `isodata`、
`rcsf.inp`、`previous.w`、`rwfn.inp` SHA-256 全部相同，但第 0 轮能量严重偏离：

| 全局索引 / J | Job 631 第 0 轮 / Eh | Job 642 第 0 轮 / Eh | 差值 / Eh |
|---|---:|---:|---:|
| 4 / 2 | -1519.468591505276 | -1585.504829436923 | -66.036237931646 |
| 7 / 3 | -1519.462944275454 | -1576.409017103590 | -56.946072828136 |
| 8 / 4 | -1519.456852581286 | -1579.266214192682 | -59.809361611396 |

`MATRIXmpi` 把 `IENDC(0:NCF)` 清零，只读取 `MYID+1:NCF:NPROCS` 的列端点，
`EMT/IROW` 只连续存放本进程的列。`SPICMVmpi` 的 `IENDC(ICOL-1)+1` 和
`INIESTmpi` 的 `JCOL(J-1)+1` 因此错误：对于多 rank，相邻全局列属于别的进程，
其端点不表示本地起点。已恢复从前一本地列的结尾累计起点，保留其余护栏改动。

新增一个 CTest `mpi90_local_sparse`，直接调用这两个生产 Fortran 例程，以
8×8 稠密已知矩阵为独立参考，覆盖不等长稀疏列、多右端项、完整/截断初始子空间，
用 1/2/3/4/46 份本地分区模拟数据存放。只替换并校验归约接口，不启动 MPI 作业。
修正前 2/3/4 份分区的矩阵乘积最大绝对误差分别为 12.614、8.529、6.857，
packed 条目误差分别为 12、10、8；修正后乘积最大误差 4.44e-16，packed 误差为 0，
特征对残差约 1.76e-15。该小矩阵分成 46 份时每份最多一列，只覆盖空分区边界，
不能替代真实 46-rank 初始 CI 验证。更新后的 `build/bin/rmcdhf_mpi` 构建成功。

还发现回退输出并非完整事务：`SCFmpi` 在检查前写 `rwfn.out`，第四次回退在重新
写恢复态之前 `ERROR STOP`，所以 Job 642 的 `rwfn.out` 等于被拒绝的 iter004，
与前三次恢复快照不同；`MATRIXmpi` 也在判定前写了 `rmix.out`。这些文件不能作为
已接受波函数继续使用，尚需修正失败出口的持久化时序。

当前后续顺序：先用修正构建核实第 0 轮与已有 Job 631 对照一致，再处理回退输出和
跨接受轮的目标身份保持，最后回到轨道步长/权重问题。Job 631 的共同 AS1 本身已
偏离 NIST（第 23–24 节），恢复其初始 CI 只代表底层回归消除，不代表中性原子的
原始轨道优化问题已经解决。本次未提交新 Slurm 作业，也未重复既有物理计算。

## 32. 回退输出事务修正与 Job 643（2026-09-13）

已完成回退失败出口的持久化修正。候选轮仍先完成 `MATRIXmpi` 和检查；若被拒绝，
程序恢复内存快照后立即重写 `.rwf`，并由 `REWRITE_ROUND_MIX` 直接用已接受快照的
`EVAL/EVEC/EAVBLK/ICCMIN` 重写 `.mix`。该写回例程不再调用 `MATRIXmpi`，因此不会
额外对角化、再次阻尼或改变回退计数。达到回退上限时，磁盘中的两个波函数文件仍是
最后一个已接受状态，随后才执行 `ERROR STOP`；默认关闭整轮护栏的历史路径未改变。

本地 `cmake --build build -j2 --target rmcdhf_mpi`、`mpi90_local_sparse` CTest、
六项离线目标态匹配测试和 `git diff --check` 均通过。已提交 Job 643（46 MPI ranks）
运行修正后的 Ni I AS2 最小护栏夹具，目标态为 `4,7,8`。该作业用于核实第 0 轮初始
CI 能量恢复到 Job 631，并检查回退后的 `.rwf/.mix`；在作业完成前不据此调整阻尼、
重叠门槛或扩大测试矩阵。

## 33. Job 643：MPI 回归消除，目标态连续但轨道未收敛（2026-09-14）

Job 643 已结束。`rmcdhf_exit=0`，但 wrapper 为 1、严格收敛检查为 1；这不是
作业崩溃，而是程序跑满 100 轮后仍未满足严格门槛。第 0 轮九个 CI 根与 Job 631
逐根完全一致，包括目标根 4/7/8 的
`-1519.468591505276/-1519.462944275454/-1519.456852581286 Eh`。因此第 31 节
定位的本地稀疏列索引错误已经在真实 46-rank 路径上消除。

三条目标根在全部 100 轮中均为 `4→4`、`7→7`、`8→8`，没有 root permutation，
最低相邻轮 CI 重叠分别为 0.991565、0.993835、0.994123；所有 round 均接受，
没有能量顺序回退。这证明本次 CI 身份跟踪对当前路径工作正常，但不能说明物理结果
正确。最终仍是 `$^3F_2<{}^3F_3<{}^3F_4$`，相对最低根的间隔为
4244.23/8440.97 cm$^{-1}$，与 Job 631 `all_new` 基本一致。护栏保持了从错误
AS1/初始状态继承的身份，不会自动把精细结构改成 NIST 顺序。

严格失败原因也已确定：迭代 25 起加权能量判据为真，目标身份一直稳定，但轨道判据
到第 100 轮仍为假；5d 轨道末轮 `SCNSTY≈1.77×10^-7` 且自动阻尼趋近 1。
`check_strict_scf.py` 的错误信息已改为明确报告“达到迭代上限且未收敛”，避免把缺少
final 行写成 trace schema 问题。Job 643 没有发生回退，因而不能直接动态验收拒绝
出口的 `.rwf/.mix` 写回；该代码路径保留本地构建验证，后续只在自然触发回退的
最小算例中核对，不为此重跑完整矩阵。

同时补齐跨接受轮的身份状态：初始目标索引保持为物理标签，若某轮合法 root swap
被接受，则保存其当前 row，下一轮按该 row 对照旧 `EVEC`；拒绝轮不推进映射。离线
测试新增“接受 swap 后持续跟踪”和“拒绝 swap 不推进”两项，共八项通过。下一项
物理测试应使用已知正确的 AS1/TF 基线，并将目标 `$^3F_J$` 设为 state-specific
集合；继续在 Job 643 的错误 AS1 基线上调重叠阈值或阻尼没有诊断价值。
已新增并完成 prepare-only 检查的
`run_ni_target_baseline_round_guard_46.sbatch`：它直接使用 Job 590 已验证为正确能序的
AS1 波函数，
只选择五个 J block 的首根，故目标全局索引为 `3,4,5`，只运行 AS2。该 reduced-ASF
集合仍含 J=0、1 的辅助首根，并非只含三个 `^3F_J`。该单个作业用于检验正确 AS1、
精简 EOL 集合与身份护栏的组合路径；不重算 AS1，
也不重复 Job 643 的 full-ASF 路径。
该脚本还复用 Job 590 的固定 `ODAMP=-0.5` 和节点进度护栏，同时开启能序回退与
严格三重收敛；已提交为 Job 644。等待其结束后只审计目标能序、逐轮映射、回退原因
和收敛门槛，不提交额外回归作业。

Job 644 在进入第 0 轮 SCF 前即失败，不能用于任何数值结论。脚本误用 `optimized`
模式，只选择 `5p/5d/4f` 的一个相对论分量，却同时要求 balanced pair；启动检查
正确报告三组 `unbalanced_selection` 并退出 1。另一个包装缺陷是脚本写完状态后未返回
`runner_exit`。现已改为 `balanced` 模式（`5p-/5p`、`5d-/5d`、`4f-/4f`）并显式
`exit $runner_exit`；shell 与 prepare-only 检查确认输入轨道为
`6s,5p-,5p,5d-,5d,4f-,4f`，修正后重新提交为 Job 645。

## 34. Job 645：已验收 balanced AS1 与 reduced-ASF 路径严格收敛（2026-09-14）

Slurm 终态为 `COMPLETED 0:0`，总耗时 35:16，其中 RMCDHF 35:05；脚本状态为
`runner_exit=0, rmcdhf_exit=0, convergence_exit=0`。计算经过 62 轮后同时满足
轨道、加权能量和目标身份判据，连续严格通过数为 2。

五个 J block 各选一根，目标物理标签为 3/4/5（J=2/3/4）。62 轮内均保持
`3→3/4→4/5→5`，最低相邻轮 CI 重叠为 0.992726/0.992766/0.992726；所有
62 个 round 均接受，未发生能序回退或 root swap。第 3 轮只有 `5p-` 一次
`nodes_progress` 轨道拒绝，与 Job 590 已知行为一致，随后正常收敛。

最终顺序为 `$^3F_4<{}^3F_3<{}^3F_2$`，相对最低级累计间隔为
1374.81/2290.63 cm$^{-1}$（相邻第二间隔 915.83 cm$^{-1}$）。相对 NIST
1332.164/2216.550 cm$^{-1}$ 的累计偏差约为 +42.65/+74.08 cm$^{-1}$。
与既有 reduced-ASF Job 602 的 1374.78/2290.42 cm$^{-1}$ 基本一致。

Job 645 是组合路径通过，不能作单因素归因。它与无整轮护栏的 reduced-ASF Job 602
在前 18 轮共有的 1671 条生产事件中，选取的 19 个身份与数值字段完全一致；Job 645 的 62 个 round 又全部
接受，因此整轮护栏没有改变这条轨道路径。Job 601 在相同正确 AS1 上使用 full ASF
同样保持正确顺序，full 与 reduced-ASF 的累计间隔只差约 1.1/1.6 cm$^{-1}$。所以
此次正确能序既不是由精简状态集合单独恢复，也不是由 CI overlap 回退产生。Job 645
真正新增的证据是：同一路径继续运行到严格轨道收敛后，能序稳定，累计间隔相对
Job 602 只变化 0.03/0.21 cm$^{-1}$。
最终 `.w` 的对照显示后 44 轮仍有小量径向精化：新增轨道与 Job 602 最终轨道的
最低重叠为 0.999923（`5d-`），最大平均半径比为 1.01274。离线 `10^{-10}`
相对幅值节点算法会把 Job 602 的 `5d-/5d/6s` 记为 3/3/6、Job 645 记为
2/2/5；但生产 trace 的 `COUNT` 在第 18 轮已经是 2/2/5。因此严格收敛主要消除了
低幅径向尾部的残余振荡，不能把离线计数差异解释为物理节点被修复。

## 35. 现有结果的统一因果分析（2026-09-14）

### 35.1 Ni I 的因果链

| 受控条件 | 结果 | 能支持的结论 |
|---|---|---|
| `_NV` AS1 + TF AS2，固定轨道 RCI（Job 635） | `J4<J3<J2`，1305.08/2168.02 cm$^{-1}$ | 未优化链给出正确且接近 NIST 的基线 |
| 优化 AS1 + TF AS2，同一固定 RCI（Job 636） | `J2<J3<J4`，相对 J4 为 -1411.04/-2730.88 cm$^{-1}$ | AS1 的轨道基底已经足以造成反转；不能把问题只归因于 AS2 新轨道 |
| 同一错误 AS1 + 优化 AS2（Jobs 631/643） | 错序扩大到约 -4244/-8441 cm$^{-1}$；目标根逐轮恒等匹配 | AS2 更新继续放大不平衡；不是 CI 根编号交换 |
| 共享 AS1 的 B1 正号单侧与 B3 成对对照 | B1 错序，B3 恢复正确顺序 | 最强的单因素证据指向原始 `nl` 单侧松弛 |
| balanced AS1 + balanced AS2，full ASF（Jobs 590/601） | `J4<J3<J2`，1375.88/2291.99 cm$^{-1}$ | 不缩小状态集合也能保持正确能序 |
| 同一 AS1，五块各首根 reduced-ASF（Job 602） | 1374.78/2290.42 cm$^{-1}$ | 状态集合只造成约 1 cm$^{-1}$ 量级变化 |
| Job 602 路径 + round guard + strict（Job 645） | 1374.81/2290.63 cm$^{-1}$，62 轮严格收敛 | 护栏未介入物理路径；严格收敛只作小量数值精化 |

原始外部 `optimized` 列表只优化 `nl`，固定对应 `nl-`；Ni I AS1 的 balanced
列表则同时优化 `4p-/4p`、`4d-/4d`，AS2 同时优化 `5p-/5p`、`5d-/5d`、
`4f-/4f`。不同 $J$ 态对这些 $\kappa$ 轨道的占据和耦合不同，单侧松弛会给各
$J$ 块带来不等的相关能降低。在截断 CSF 空间中，轨道旋转不具有完整 CI 的不变性，
所以 EOL 变分降低加权总能量，并不保证精细结构间隔或能级顺序更接近实验。

这也解释了 Thomas--Fermi 初猜看起来更好。TF 轨道来自共同中心场，径向形状较平滑、
弥散，并且没有经过针对当前有限状态集合的 $J$ 依赖松弛，因而在有限相关模型中相当于
一个正则化的公共轨道基组。它保留了正确差分能量并可能与尚未包含的高阶相关、Breit/QED
修正形成误差抵消。TF 本身不是普遍更准确：Job 636 已证明，把 TF AS2 接在错误的优化
AS1 上，Ni I 仍然倒序。关键是整条基线及松弛是否平衡，而不是新轨道的生成器名称。

### 35.2 当前整轮护栏能做什么

整轮护栏现在可以阻止一次候选轮造成的低重叠根丢失或相对能序突变，并在回退后恢复
内存和输出文件；严格收敛可以防止只因加权能量先稳定就提前结束。Jobs 643/645 中没有
一次整轮回退，目标匹配也全部恒等，因此这两个作业没有证明回退能修复原始物理异常。
Job 645 每个 J block 只选一根，块内 assignment 本身是平凡的；合法 root swap 的跨轮
持久映射目前只有八项离线测试覆盖。

更关键的是，当前 overlap 比较相邻接受轮。物理态可以每轮变化很小、长期累积成另一种
组态而始终通过相邻重叠门槛。能序护栏也只保持进入本轮时的顺序；若 AS1 已经错序，
Job 643 这类路径会稳定地保持错误顺序。因此它是安全诊断，不是错误基线的恢复算法。
实现中的数值还是相同 CSF 排列下新旧 `EVEC` 系数的绝对点积；两轮 `PF/QF` 已不同，
未做双正交轨道变换，所以它不是严格的多电子 ASF 重叠。仓库当前没有可直接复用的
双正交 ASF 变换程序。
目前有效的 Jobs 631/635/636 中目标仍标为同一 `^3F_J`，主导权重约 0.90--0.99，
没有复现“直接变成另一组态”；Job 609 的谱项损坏来自已撤销的稀疏索引回归，不能
作为该物理现象的证据。

若要覆盖“谱项逐渐变成其他组态”，后续实现应增加不随 round 更新的初始 TF/AS1
锚点，并同时记录：目标对子空间的主角重叠、参考组态 CSF 权重以及最终 LSJ/主导组态。
近简并根应比较子空间主角而非强制单根编号。该增强应先做离线/小型状态逻辑测试；
Ni I 的现有 Jobs 590/601/602/635/636/643/645 已足以支持因果结论，不再重复提交。

### 35.3 当前可执行结论

生产工作流首先应强制检查同一 $n,l$ 的 relativistic partners 是否成对变分，并拒绝
把未经物理验收或已错序的 AS 波函数作为下一活性空间基线。新活性空间先保存 TF 固定
轨道结果作为锚点；需要优化时使用 balanced 列表、受控阻尼和严格收敛，再与锚点比较
能序、间隔、主导组态和径向指标。

Job 645 的累计 NIST 偏差仍约 +42.65/+74.08 cm$^{-1}$。full/reduced-ASF 的差别
远小于该偏差，三档阻尼也只造成小量间隔变化，所以现有数据不支持继续靠状态集合、
overlap 阈值或更强阻尼消除它。剩余偏差更可能来自有限活性空间中的相关平衡和未包含的
物理修正；需要扩展相关模型后用固定轨道 RCI 作单因素比较，而不是继续修改身份护栏。

### 35.4 跨体系适用边界

| 体系 | 原始 `nl` 单侧优化 | balanced 证据 | 可得结论 |
|---|---|---|---|
| Ni I | B1、Jobs 618/631/643 错序，错误 AS1 会被后续 AS 继承 | B3、Jobs 590/601/602/645 均保持 `J4<J3<J2` | 伙伴选择方向是主要触发因素；balanced 能稳定能序，但间隔仍偏离 NIST |
| Ni/Ca-like | B1/Job 619 在 AS2 产生 `J2<J4<J3` 塌缩 | B2/B3 与 Job 579 保持 `J2<J3<J4` | 单侧松弛与多新增轨道联合造成 J 依赖能量重排；无 CI 根交换证据 |
| Cl I | 同一 AS1 夹具的 B1 为 `J1/2<J3/2`，间隔 -2479.51 cm$^{-1}$ | B2/B3 恢复 `J3/2<J1/2`；B3 间隔 932.19 cm$^{-1}$，Job 580 的 AS2--AS5 继续保持该顺序 | 原始 `nl` 单侧方向触发倒序，balanced 可恢复并跨 AS 保持；约 49 cm$^{-1}$ 的 NIST 偏差仍存在 |
| Fe I | Job 621 单侧 AS1--AS2 仍保持 `J4<J3<J2<J1<J0` | 尚无同输入 balanced 单因素 | 不平衡选择是高风险条件，但不是每个体系必然失败的充分条件 |

因此可以把“相对论伙伴完整性检查”推广为生产安全约束，却不能声称所有单侧运行
一定错，也不能声称 balanced 后必然比 TF 更接近 NIST。Ni I 和 Cl I 中，未优化
基线的间隔反而略优于 balanced 优化；Ni/Ca-like 则从 balanced 优化中得到明显改善。
优化是否提高精度必须逐体系、逐活性空间与固定 TF/RCI 锚点比较。

### 35.5 方案完成度与剩余工作（2026-09-15 更新）

已完成且无需重复的工作：

1. B0--B8 的轨道选择、阻尼、状态集合及固定轨道 RCI 判别；
2. Ni I/Ni/Ca-like 的 AS1 与 AS2 贡献分解，以及逐新增轨道隔离；
3. QDIF、`MPI_Gatherv`、稀疏列索引的单因素审计；修正后的生产 46-rank 初始 CI
   已由 Job 643 与 Job 631 逐根一致验证；
4. 节点进度判据、balanced-pair 启动检查、严格三重收敛、相邻轮 CI assignment、
   整轮内存/`.rwf`/`.mix` 回退以及跨接受轮目标 row 持续跟踪；
5. Job 645 对正确 AS1 + balanced AS2 路径的 62 轮严格收敛验证。

原“剩余工作”第 1--3 项现已完成：

1. **生产输入闸门。** 生成 varied list 的工作流默认要求同一 $n,l$ 的 $nl-/nl$
   同时变分；若显式要求单侧实验，必须标记为诊断模式。核心程序中的历史默认不应
   静默改写用户输入。
2. **固定参考锚点。** 每个 AS 开始时保存 TF 或已验收 AS。循环内计算相对
   固定参考的 CI 系数子空间主角和径向轨道重叠矩阵，但必须标为代理量；不同轨道基组
   下的严格 ASF 重叠需要双正交变换，当前仓库尚无该能力。近简并根按整个目标子空间
   观察，阶段末以固定轨道 RCI、LSJ/主导组态和参考组态权重作硬验收。这样才能检测
   “小步累计成另一组态”，又不会把系数点积误称为物理波函数重叠。
3. **阶段验收与整体回退。** 每个新 AS 先生成固定 TF 的 CI/RCI 基线；优化结果若
   改变目标谱项、预期能序或超过明确的径向/间隔容差，则该 AS 整体回到 TF/上一已验收
   波函数。能序应作为验收条件，不能在 Hamiltonian 中人为强迫成 NIST 顺序。
   TF 基线本身也必须先通过目标 LSJ/宇称、主导组态/权重和能序闸门；失败时状态为
   `rejected_baseline`，回退到上一已验收波函数且不启动 balanced RMCDHF。
生产入口位于 `graspkit-tools/scripts/grasp_regular_cal/`。生成的 `mcdhfmpi.sh`
严格按递增 AS 调用 `run_orbopt_stage.py`；每个 stage 将 CSF、上一已验收波函数、
isodata、验收规则、运行参数、TF 波函数/固定 RCI 基线以及受控 `rmcdhf_mpi` 副本
哈希冻结到 `anchor/`；受控二进制还必须声明 balanced、round rollback、固定参考及
anchor trace 能力，否则在建 stage 前拒绝。只有通过严格收敛、固定参考目标身份、
LSJ/宇称/主导组态/权重、
能序、径向代理与配置物理阈值的候选才原子发布到 `accepted/`；失败统一进入
`rejected_*` 并让 `next_restart.json` 指回不可变 anchor。下一 AS 的准备函数重新核验
上一 stage 的 accepted manifest、文件大小与 SHA-256，拒绝 candidate 或 fallback 冒充。

仍需保留的后续科学/运行验证为：

1. **动态回退文件验收。** 当前 `.rwf/.mix` 事务修复已构建并离线核查，但 Jobs
   643/645 没有触发 round rollback。只在后续自然出现有效回退时核对文件，不为取得
   这一覆盖重复提交现有 Ni I 计算。
2. **精度模型。** 在上述安全闸门稳定后，再用扩展相关空间、Breit/QED 等独立物理
   轴解释剩余 NIST 偏差；每次只改一个物理因素并用固定轨道 RCI 对照。

因此，生产事务机制已经具备针对本方案目标态集合的缓慢漂移和“谱项直接变为其他
组态”自动验收/回退能力；这里的 CI 系数与径向 overlap 仍明确属于代理量，不能宣称
为不同轨道基组间的严格多电子 ASF 重叠，也不能替代逐体系物理精度验证。
