...
46-rank 稀疏矩阵回归以及整轮身份/回退状态机均已有结果。尚未完成的是固定参考
锚点与组态漂移门槛、有效物理算例中的动态回退输出验收，以及四体系 balanced
生产流程的最终覆盖；串行/MPI 逐轮对照不作为本方案验收项。当前结论与完成度见
第 35 节。具体状态也可对照
...
1. **生产输入闸门。** 由生成 varied list 的工作流默认要求同一 $n,l$ 的 $nl-/nl$
   同时变分；若显式要求单侧实验，必须标记为诊断模式。核心程序中的历史默认不应
   静默改写用户输入。
2. **固定参考锚点。** 在每个 AS 开始时保存 TF 或已验收 AS。循环内可同时计算相对
...
4. **动态回退文件验收。** 当前 `.rwf/.mix` 事务修复已构建并离线核查，但 Jobs
   643/645 没有触发 round rollback。只在后续自然出现有效回退时核对文件，不为取得
   这一覆盖重复提交现有 Ni I 计算。
5. **精度模型。** 在上述安全闸门稳定后，再用扩展相关空间、Breit/QED 等独立物理
   轴解释剩余 NIST 偏差；每次只改一个物理因素并用固定轨道 RCI 对照。

因此，本方案已经能诊断并避免这批数据中最主要的错误路径，但还不能保证任意中性
原子的目标谱项绝不发生缓慢漂移。完成第 2、3 项后，才具备针对用户所述“谱项直接
变为其他组态”的完整自动验收能力。
