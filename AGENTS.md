...
- `grasp/` contains a local copy of the GRASP program source. It is kept as a reference for understanding upstream GRASP behavior and interfaces when making changes in the paired repositories. It is reference material only: it is not imported by the Python/Rust packages and is not part of the workspace build or dependency graph.
